...
// ---------- XỬ LÝ XÓA (dùng StudentController — tự cập nhật giường trống) ----------
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete') {
    $delId = filter_input(INPUT_POST, 'student_id', FILTER_VALIDATE_INT);
    if ($delId) {
        $result = $controller->deleteStudent($delId);
        // Lưu thông báo tạm vào session để hiện sau khi redirect
        session_start();
...
// ---------- LẤY DANH SÁCH SINH VIÊN (qua StudentController, PDO Prepared Statement) ----------
...
:root{
    --bg:#f0f2f5;--card:#fff;--primary:#0ea5a4;--primary-dk:#0f766e;--primary-lt:#e6fffa;
...
.btn-edit{background:var(--primary-lt);color:var(--primary-dk);border:1px solid #99f6e4}.btn-edit:hover{background:#ccfbf1}
...
<main class="page">
    ...
    <div style="padding:12px 18px;border-radius:8px;margin-bottom:20px;font-size:14px;
                background:<?= $deleteMessage['success'] ? '#dcfce7' : '#fee2e2' ?>;
                color:<?= $deleteMessage['success'] ? '#16a34a' : '#dc2626' ?>">
